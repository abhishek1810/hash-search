...
int main() {
    ...
    struct hashObject *bucket = (struct hashObject *) malloc( hashesPerBucketRead * sizeof(struct hashObject) );
    ...
    // for (int i = 0; i < 1; i++)
    {
        ...
        size_t numberOfSortedBuckets = hashesPerBucketRead < maxHashesToSort ? 1 : hashesPerBucketRead / maxHashesToSort;

        for (size_t j = 0; j < numberOfSortedBuckets; j++)
        {
            //STEP 2 : Seek to the position in file
            size_t bucket_location = (prefix * hashesPerBucketRead + j * numberOfHashesInBucket) * sizeof(struct hashObject)  ;
            ...
            size_t bytesRead = fread(bucket, 1, numberOfHashesInBucket * sizeof(struct hashObject), file);
            ...
            int search_result = binarySearch( bucket, 0, numberOfHashesInBucket-1, &array[i][PREFIX_SIZE]);
            if ( search_result != -1 ) {
                found = true;
                break;
            }
