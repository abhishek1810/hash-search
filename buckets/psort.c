...
void *sortBucket(void *arg)
{
    ...
    struct hashObject *bucket = &buckets[thread_id * numberOfHashesInBucket];
...
int main() {
    ...
    bucketSizeInBytes = numberOfHashesInBucket * sizeof(struct hashObject);
    ...
    for (size_t i = 0; i < (numberOfBucketsToSort / NUM_THREADS); i++)
    {
