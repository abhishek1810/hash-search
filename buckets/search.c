...
int main() {
    ...
    // for (int i = 0; i < 1; i++)
    {
        ...
        size_t bucket_location = prefix * hashesPerBucketRead * sizeof(struct hashObject);
        ...
        size_t bytesRead = fread(bucket, 1, hashesPerBucketRead * sizeof(struct hashObject), file);
        if (bytesRead != hashesPerBucketRead * sizeof(struct hashObject))
        {
        ...
        bool found = false;
        for (size_t j = 0; j < hashesPerBucketRead; j++)
        {
